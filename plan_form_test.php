...
if(isset($_POST['upload_excel']))
  ...
  include 'PHPExcel/IOFactory.php';
      ...
      for($i=2;$i<=$arrayCount;$i++){
        ...
        //   $plan->rajpatra_no = $_POST['rajpatra_no'];
        //   $plan->qty = $_POST['qty'];
        //   $plan->topic_no = $_POST['topic_no'];
		  ...
		  $plan->rajpatra_no = trim($allDataInSheet[$i]["H"]);
		  $plan->topic_no = trim($allDataInSheet[$i]["I"]);
		  $plan->topic_area_id = trim($allDataInSheet[$i]["J"]);
		  $plan->topic_area_type_id = trim($allDataInSheet[$i]["K"]);
		  $plan->budget_id = trim($allDataInSheet[$i]["L"]);
		  $plan->qty = trim($allDataInSheet[$i]["M"]);
		  $plan->type =$_POST['type'];
		  $plan->save();
	  }
...
<div class="">
 ...
 <span><a href="sample/yojana_sample.xlsx" type="download" class="btn">स्याम्पल डाउनलोड</a></span>
            </form>
                </div>
                  </div>
                </div><!-- main menu ends -->
