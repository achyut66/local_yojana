...
//                echo array_shift($row);exit;
        ...
        public static function count_by_budget_id($budget_id,$fiscal_id,$ward)
	{
        ...
        public static function get_by_topic_area_id($clause, $topic_area_id,$fiscal_id,$ward) {
					global $database;
            if(empty($ward)) {
            	$sql = "select * from ".self::$table_name." where $clause=".$topic_area_id." and fiscal_id=".$fiscal_id;
            } else {
            		$sql = "select * from ".self::$table_name." where ward_no=".$ward." and $clause=".$topic_area_id." and fiscal_id=".$fiscal_id;
              }
							$result = self::find_by_sql($sql);
							return $result;
	}
        public static function count_by_topic_area_id($clause, $string,$fiscal_id,$ward)
	{
		global $database;
                if(empty($ward))
                {
                    $sql = "select sum(investment_amount) as total_investment, count(*) as total_plans from ".self::$table_name." where $clause=".$string." and fiscal_id=".$fiscal_id;
		
                }
                else
                {
                    $sql = "select sum(investment_amount) as total_investment, count(*) as total_plans from ".self::$table_name." where ward_no=".$ward." and $clause=".$string." and fiscal_id=".$fiscal_id;
