    ...
    foreach ($result as $data) {
        if ($data->type==0) {
            ...
            if (!empty($budget)) {
                ...
                $sql = "select  sum(pa), sum(nta) from (\n"

                . "select program_payment.program_id, program_payment.payment_amount as pa, program_payment_final.net_total_amount as nta  \n"
            
                . "from program_payment left outer join program_payment_final on program_payment.program_id = program_payment_final.program_id where program_payment.program_id = ".$data->id."\n"
            
                . "union all\n"
            
                . "select program_payment_final.program_id, program_payment.payment_amount as pa, program_payment_final.net_total_amount as nta \n"
            
                . "from program_payment right outer join program_payment_final on program_payment.program_id = program_payment_final.program_id where program_payment_final.program_id = ".$data->id."\n"
                ...
                $net_payable_amount = $row[0] + $row[1];
            }
        }
...
function getAbstractNapiDashHtml($max_period)
{
    $total_count = $max_period+1;
    $html = '';
    $last_flag = '';
    ...
    if ($last_flag==1) {
        for ($i=1;$i<=$max_period;$i++) {
            if ($i==$max_period) {
                $html .= '<a href="abstract_napi_lagat_anuman.php?id='.$_SESSION['set_plan_id'].'&period='.$i.'"><div class="userprofile">
                                <h3>अन्तिम  हेर्नुहोस / सच्याउनुहोस</h3>
                                <div class="dashimg">
                                    <img src="images/upabhokta-icon.png" alt="Upabhokta Icon" class="dashimg" />
                                </div>
...
function getMaxLetterNo($cert)
{
    ...
    if ($cert==2) {
        $letter = Taxlabour::count_all();
        $letter=$letter+1;
    }
    if ($cert==3) {
        $letter = Taxpersonal::count_all();
        $letter=$letter+1;
    }
    if ($cert==4) {
        $letter = Taxrenewal::count_all();
        $letter=$letter+1;
    }
    if ($cert==5) {
        $letter = Taxdisc::count_all();
        $letter=$letter+1;
    }
    return $letter;
}
