...
class AbstractCost
{
    protected static $table_name = "abstract_cost";
    protected static $db_fields = array('id','name','quantity','unit_id','rate','total','plan_id', 'version');
    public $id;
    public $name;
    public $quantity;
    public $unit_id;
    ...
    {
        global $database;
        $result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$id} order by id asc");
        return $result_array;
    }
    public static function find_by_plan_version($id=0, $version = 0)
    {
        global $database;
        $result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$id} and version={$version} order by id asc");
        return $result_array;
    }
