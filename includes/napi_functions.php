<?php
    ...
    function getNapiView($period)
    {
        ...
        if ($lagat_detail->break_type==2) {
            ...
            $html .='</td>
                                        <td class="mycenter">'.Units::getName($lagat_detail->unit_id).'</td> 
                                        <td>'.$break_lagat->task_rate.'</td>
                                        <td>'.$break_lagat->total_rate.'</td>
                                        <td></td>
                             </tr>';
            $j++;
            endforeach;
    ...
    function getAbstractNapiView($period)
    {
        $profile_details = NapiLagatProfile::find_by_plan_id_period($_GET['id'], $period);
        $max_period = NapiLagatProfile::find_max_period($_GET['id']);
        $del_link = '';
        ...
        $html .= '<span class="myPrint"><a href="abstract_print_estimate_naapi_final.php?id='.$_GET['id'].'&period='.$period.'" target="_blank">प्रिन्ट गर्नुहोस</a></span>';
        ...
        endforeach;
        $html .='<tr>
                                    <td class="mycenter" colspan="5" style="text-align:right">कुल जम्मा रु</td>
                                    <td class="mycenter">'.$profile_details->sub_total.'</td>
                                    <td></td>
                                </tr>
    ...
    function getAbstractNapiLetter($period)
    {
        $profile_details = NapiLagatProfile::find_by_plan_id_period($_GET['id'], $period);
        $sql = "select * from napi_lagat_anuman where plan_id=".$_GET['id']." and period=".$period." order by sno asc";
        $lagat_details = Napilagatanuman::find_by_sql($sql);
