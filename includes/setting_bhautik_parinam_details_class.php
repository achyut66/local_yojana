...
class SettingbhautikPariman
{
    ...
    protected static $db_fields = array('id','name','unit_name','parent_id');
    public $id;
    public $name;
    public $parent_id;
    public $unit_name;
    ...
    public static function get_report_data($parent_id) {
        global $database;
        $sql = "SELECT t1.details_id,t1.unit_id,sum(t1.prev_qty),t1.plan_id,t1.type,sum(t1.qty) as total_qty,t2.name as tname FROM bhautik_lakshya t1 left join setting_bhautik_pariman t2 on t2.id = t1.details_id WHERE t1.details_id =" .$parent_id ." group by t1.details_id";
        $result_set = $database->query($sql);
