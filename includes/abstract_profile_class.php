...
{
    protected static $table_name = "abstract_profile";
    protected static $db_fields = array('id','sub_total','date_nepali','plan_id','in_review','approved','created_by', 'approved_by', 'version');
    public $id;
    public $sub_total;
    public $date_nepali;
    public $plan_id;
    ...
    public static function find_by_plan_version($id=0)
    {
        global $database;
        $result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$id} order by version desc limit 1");
        return !empty($result_array)? array_shift($result_array) : false;
    }
