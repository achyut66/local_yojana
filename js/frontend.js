...
JQ(document).ready(function () {
  ...
  JQ(document).on('change', '#budget_id', function () {
    ...
    JQ.post('get_remaining_budget.php', param, function (res) {
      ...
      if (budget_amount < total_investment) {
        ...
        alert('बजेट रकम भरिएको छैन ,कृपया सेटिंगमा गएर बजेट रकम भर्नुहोस्');
        return false;
      } else {
  ...
  JQ(document).on(
    'input',
    '.remaining_sample,.final_deducted_one,.final_one,.due_one,.disaster_one',
    function () {
      ...
      var total_sample_deducted =
        parseFloat(final_one) + parseFloat(due_one) + parseFloat(disaster_one);
  ...
  //     alert("here");
  //     var num=JQ("#remove_more_1").length;
  //     var counter=num+2;
  //     // var ward = JQ("#ward").val();
  //     //alert(ward);
  //     //alert(counter);
