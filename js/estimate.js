...
JQ(document).ready(function () {
  ...
  JQ(document).on('click', '#calc_close', function () {
    ...
    if (!isNaN(counter)) {
      ...
      if (type === 2) {
        ...
        var overhead = parseFloat(sub_total * 0.15);
        var vat_amount = parseFloat((sub_total + overhead) * 0.13);
  ...
  JQ(document).on(
    'input',
    "input[name~='length[]'], input[name~='breadth[]'],input[name~='height[]'],input[name~='total_evaluation[]'],input[name~='task_rate[]'],input[name~='task_count[]']",
    function () {
  ...
  JQ(document).on(
    'input',
    "input[name^='length-'], input[name^='breadth-'],input[name^='height-'],input[name^='total_evaluation-'],input[name^='task_count-']",
    function () {
      var id_selected = JQ(this).attr('id');
  ...
  //   JQ(document).on(
  //     'input',
  //     '#investment_amount,#other_source,#samiti_investment,#other_agreements',
  //     function () {
  ...
  //         parseFloat(other_source) +
  //         parseFloat(samiti_investment) +
  //         parseFloat(other_agreements);
  ...
  JQ(document).on('click', '.get_measurement', function () {
    ...
    var html =
      '<input type="hidden" name="estimate_sub_id[]" value="' +
      estimate_sub_id +
      '" />' +
      '<span class="change_estimate_sub" id="counter-' +
      counter +
      '">' +
      estimate_sub_name +
      '</span>';
    ...
    if (task_name == '') {
      ...
      JQ.post('get_estimate_task_unit.php', param, function (res) {
        ...
        var html =
          ...
          '</div>' +
          '<div class="modal-body" style="font-size:14 px">' +
          estimate_html +
          '</div>' +
          '<div class="modal-footer">' +
          '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>' +
  ...
  JQ(document).on('click', '.break', function () {
    ...
    if (break_row_length > 0) {
      var test_row_id = JQ('.' + break_row_class)
        .last('tr')
        .attr('id');
    ...
    JQ.post('getmeasurementdiv.php', param, function (res) {
      ...
      if (break_row_length == 0) {
        ...
        //change
        var break_row_class = 'break_row-' + counter;
        var break_row_id = JQ('.' + break_row_class)
          .last('tr')
          .attr('id');
