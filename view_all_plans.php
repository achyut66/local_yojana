...
//    echo $topic_area_investment_id;exit;
  ...
  if(empty($_GET['id']))
  {
    ...
    if(!empty($topic_area_id))
    {
      strpos($sql, 'where') === false ? $sql .= " where " : $sql .= " and";
      $sql .=" topic_area_id='".$topic_area_id."'";
    }
    if(!empty($budget_id))
    {
      strpos($sql, 'where') === false ? $sql .= " where " : $sql .= " and";
      $sql .=" budget_id='".$budget_id."'";
...
<body>
  ...
  <div id="body_wrap_inner">
    <div class="maincontent">
          ...
          <?php if($user->mode!="user" && $user->mode!="section"):?>
            <form method="get">
              <table class="table table-bordered table-responsive" style="table-layout: auto;table-border:2px;">
                <tr>
                 <td class="myCenter" width="10%">किसिम छान्नुहोस्
                 ...
                 <td class="myCenter" width="20%"> बिषयगत क्षेत्रको किसिम:<select name="topic_area_id" class="form-control">
                   <option value="">-छान्नुहोस्-</option>
                 ...
                 <td class="myCenter" width="20%"> बजेट उपशिर्षक:<select name="budget_id" class="form-control">
                   <option value="">-छान्नुहोस्-</option>
                   <?php foreach($budget_result as $topic): ?>
                     <option value="<?php echo $topic->id?>" <?php if($topic->id==$budget_id){?> selected="selected"<?php } ?>><?php echo $topic->name;  ?></option>
                   <?php endforeach; ?>
                 </select>
               </td>
                 <td class="myCenter" width="20%"> योजनाको विनियोजन किसिम:<select name="topic_area_investment_id" class="form-control">
                   <option value="">-छान्नुहोस्-</option>
               ...
               <td class="myCenter" width="10%">वार्ड नं :<input type="text" name="search_ward" class="input100percent" value="<?php echo $search_ward;?>"/></td>
               <td class="myCenter" width="10%">दर्ता नं : <input type="text" name="search_text" class="input100percent" value="<?php echo $search_text;?>"/></td>
               <td>योजनाको नाम </td>
               <td><textarea name="program_name" ><?php echo $program_name;?></textarea></td>
               <td class="myCenter"><input type="submit" name="submit" value="Submit" class="btn-success" /><br>
               <div class="myspacer"></div>
               <a href="view_all_plans.php"><input type="button" class="btn-danger" value="Cancel" /> </a></td>
             </tr>
           </table>   
         </form>
       <?php endif;?>
       <?php if($user->mode=="section" || $user->mode=="superadmin" || $user->mode=="administrator" || $user->mode=="user"):?>
        <td>
          <div class="text-right" style="margin-top:-15px;">
            <a href="print_view_all_plans.php?type=<?=$_GET['type']?>&topic_area_id=<?=$_GET['topic_area_id']?>&budget_id=<?=$_GET['budget_id']?>&topic_area_investment_id=<?=$_GET['topic_area_investment_id']?>&search_text=<?=$_GET['search_text']?>&ward=<?=$_GET['search_ward']?>&topic_area_id=<?=$user->topic_area_id?>&topic_area_type_id=<?=$user->topic_area_type_id?>" target="_blank">
              <input type="button" class="btn-primary" value="प्रिन्ट गर्नुहोस">
            </a>
          </div>
        </td>
        <?php endif;?>
      ...
      $d_activate = DeactivePlan::find_by_plan_id($data->id);
      // print_r($d_activate);
//      echo "<pre>";
//      print_r($if_samiti_bhuk);
     ...
     //print_r($data);
     ?>
     <tr <?php if(!empty($d_activate)){?>style="background-color: #dc3545; color:#fff;"<?php }?>)>
     <?php if(!empty($d_activate)){?>
          <td>
      ...
      <?php }?>
      <?php if(!empty($d_activate)){?>
        <?php $link_for = "plan_form_edit.php?id=".$data->id;
        ?>
        <td>
        <a href="<?=$link_for?>" data-toggle="tooltip" style="color:white;" data-placement="top" title="<?php echo $d_activate->deactivate_reason;?>" value="<?php echo $data->id;?>">
        <?php echo $data->program_name;?>
      </a>  
        </td>
      <?php }else{?>
        <td>
        <!-- <a href="" data-toggle="tooltip" style="color:blue;" data-placement="top" title="उपभोक्ता समिति" value="<?php echo $data->id;?>">
        <a href="" data-toggle="tooltip" style="color:red;" data-placement="top" title="उपभोक्ता समिति" value="<?php echo $data->id;?>">
        <a href="" data-toggle="tooltip" style="color:black;" data-placement="top" title="उपभोक्ता समिति" value="<?php echo $data->id;?>">
        <a href="" data-toggle="tooltip" style="color:blue;" data-placement="top" title="उपभोक्ता समिति" value="<?php echo $data->id;?>"> -->
        <!-- <a href="set_id.php" data-toggle="tooltip" style="color:blue;" data-placement="top" title="उपभोक्ता समिति" value="<?php echo $data->id;?>">
        <a href="set_id.php" data-toggle="tooltip" style="color:blue;" data-placement="top" title="अमानत" value="<?php echo $data->id;?>"> -->
          <?php echo $data->program_name;?>
          </a>
        </td>
      <?php }?>
      ...
      <td><a href="<?php echo $link; ?>"><input type="button" class="btn-primary" value="पुरा विवरण"></a></td>
    </tr>
    <?php $i++;
